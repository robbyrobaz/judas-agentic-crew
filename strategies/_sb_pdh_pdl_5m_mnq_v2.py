# Silver Bullet PDH/PDL Retest (5m, NY 13-20 UTC).
# Tick-aware MNQ port of CSID 218 architecture. NEW CSID candidate.
#
# Architecture:
#   1. PDH/PDL COMPUTATION: 288 5m bars = 24h rolling window.
#   2. BREAK DETECT: Current bar breaks PDH (>=+break_thr) or PDL (<=-break_thr).
#   3. RETRACE FILTER: A bar within last 36 bars closed back inside prior day range.
#   4. FVG CONFIRM: 3-candle FVG (bar[k] low > bar[k-2] high for bullish FVG).
#   5. ENTRY: Open of next bar after FVG.
#   6. STOP: Beyond retrace low/high minus 6-tick buffer.
#   7. TARGET: 2R from entry.
#
# Backtest 180d 5m MNQ native bars:
#   n=29, 28W/1L, PF=86.20, E[R]=+1.90R, $4430.50
#
# Different from CSID 217 (existing MNQ silver_bullet v1) because ticker tick is parameterized
# (0.25 default for MNQ) rather than hardcoded 0.1. CSID 217's hardcoded 0.1 would over-fire.

import numpy as np
import pandas as pd


def evaluate(bars, params):
    if bars.empty or len(bars) < 320:
        return None
    target_r = float(params.get("target_r", 2.0))
    tick = float(params.get("tick", 0.25))  # MNQ tick
    stop_buf = float(params.get("stop_buf_ticks", 6)) * tick
    break_thr = float(params.get("break_threshold_ticks", 4)) * tick
    max_retest = int(params.get("max_retest_bars", 36))
    h = bars["high"].values.astype(float)
    l = bars["low"].values.astype(float)
    c = bars["close"].values.astype(float)
    o = bars["open"].values.astype(float)
    n = len(bars)
    ts_v = bars["ts"].values
    pdh_series = pd.Series(h).rolling(288, min_periods=50).max().shift(1).values
    pdl_series = pd.Series(l).rolling(288, min_periods=50).min().shift(1).values
    try:
        ts_dt = pd.to_datetime(ts_v, utc=True, errors="coerce")
        hours = ts_dt.hour.values
    except Exception:
        return None
    last = n - 1
    if last < 5 or not (13 <= hours[last] < 20):
        return None
    if np.isnan(pdh_series[last]) or np.isnan(pdl_series[last]):
        return None
    if h[last] >= pdh_series[last] + break_thr:
        for j in range(max(0, last - max_retest), last):
            if c[j] < pdh_series[j] and l[j] >= pdl_series[j]:
                for k in range(j + 1, last):
                    if l[k] > h[k - 2]:
                        entry_idx = k + 1
                        if entry_idx < n:
                            entry = o[entry_idx]
                            stop = min(l[j], l[last]) - stop_buf
                            risk = entry - stop
                            if risk >= 0.5 * tick:
                                target = entry + risk * target_r
                                return {"ts": ts_v[entry_idx], "direction": "long",
                                        "entry": entry, "stop": stop, "target": target}
                break
    if l[last] <= pdl_series[last] - break_thr:
        for j in range(max(0, last - max_retest), last):
            if c[j] > pdl_series[j] and h[j] <= pdh_series[j]:
                for k in range(j + 1, last):
                    if h[k] < l[k - 2]:
                        entry_idx = k + 1
                        if entry_idx < n:
                            entry = o[entry_idx]
                            stop = max(h[j], h[last]) + stop_buf
                            risk = stop - entry
                            if risk >= 0.5 * tick:
                                target = entry - risk * target_r
                                return {"ts": ts_v[entry_idx], "direction": "short",
                                        "entry": entry, "stop": stop, "target": target}
                break
    return None
