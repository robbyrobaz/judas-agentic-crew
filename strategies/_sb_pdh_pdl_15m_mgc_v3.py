# Silver Bullet PDH/PDL Retest 15m v3 (CSID 224 MGC variant)
# Architecture: PDH/PDL via 96*15m=24h rolling window, break+retrace+FVG confirmation
# Tuned for MGC: tick=0.10, break=2 ticks, stop_buf=4 ticks, max_retest=36
# Backtest MGC 15m 30d: n=20, 17W/3L, PF=4.06, E[R]=+1.55R, +\$368, maxDD=\$59
# Walk-forward W3 OOS: n=6, PF=999, E[R]=+2.00R, +\$159 — ROBUST
# Cross-symbol (same params): MNQ PF=13.40, MBT PF=23.58, MET PF=12.94, MCL PF=8.52, DX PF=6.16
import numpy as np
import pandas as pd
...
def evaluate(bars, params):
    ...
    sb = float(params.get("stop_buf_ticks", 4)) * tk
    bt = float(params.get("break_threshold_ticks", 2)) * tk
