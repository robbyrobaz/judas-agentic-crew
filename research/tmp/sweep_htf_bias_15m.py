"""
Sweep entry at PRIOR-SESSION H/L on 15m + HTF BIAS FILTER.
Strategy: take the wick-sweep entry, but ONLY enter counter-trend (against sweep)
when HTF is ranging. When HTF trends strongly, skip the trade (would get run over).
"""
import numpy as np
...
def evaluate(bars, params):
    ...
    htf_lookback = int(params.get('htf_lookback', 26))  # 6.5h on 15m
    htf_atr_lookback = int(params.get('htf_atr_lookback', 26))
    max_htf_range_atr = float(params.get('max_htf_range_atr', 2.5))  # skip if HTF trending strongly

    if n < htf_lookback + session_bars + atr_period + 10:
        return None
    ...
    # HTF regime filter: compute rolling range over htf_lookback bars, in ATR units
    htf_lo = cur_i - htf_lookback
    if htf_lo < 0: return None
    htf_range = (max(highs[htf_lo:cur_i+1]) - min(lows[htf_lo:cur_i+1]))
    htf_avg_range = np.mean([highs[j]-lows[j] for j in range(cur_i-htf_atr_lookback, cur_i+1)])
    if htf_avg_range <= 0: return None
    htf_range_atr = htf_range / atr  # how many ATRs the HTF range covers
    if htf_range_atr > max_htf_range_atr:
        # HTF trending strongly — skip
        return None
    ...
    sb_h = highs[sweep_bar]; sb_l = lows[sweep_bar]; sb_c = closes[sweep_bar]

    if short_setup:
        if closes[cur_i] >= prior_hi:
            return None
        entry = sb_c; stop = sb_h + atr * stop_atr
        risk = stop - entry
        ...
        if closes[cur_i] <= prior_lo:
            return None
        entry = sb_c; stop = sb_l - atr * stop_atr
        risk = entry - stop
        if risk <= 0: return None
        target = entry + target_r * risk
