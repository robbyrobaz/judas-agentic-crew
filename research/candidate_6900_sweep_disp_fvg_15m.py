"""MGC 15m — Sweep + Displacement + FVG Midpoint Entry (tR=1.5)
Candidate ID 6900. Walk-forward avg test PF=2.89 / E[R]=+0.72 (3 windows).
Architecture: ICT-style sweep of prior N-bar extreme + displacement (range >= 1.5x ATR, body >= 0.7)
+ FVG midpoint entry. Stop beyond sweep extreme + 0.15x ATR buffer. Target 1.5R.
"""
...
def evaluate(bars, params):
    ...
    sweep_lookback = int(params.get('sweep_lookback', 12))
    ...
    if is_bull and not is_sweep_long:
        return None
    if not is_bull and not is_sweep_short:
        return None
