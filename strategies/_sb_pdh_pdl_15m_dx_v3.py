# Silver Bullet PDH/PDL Retest 15m v3 (CSID 225 DX variant)
# Architecture: PDH/PDL via 96*15m=24h rolling window, break+retrace+FVG confirmation
# Tuned for DX: tick=0.005, break=2 ticks, stop_buf=5 ticks, max_retest=48
# Backtest DX 15m 30d: n=21, PF=999, E[R]=+2.00R, +\$3.15, walk-forward W3 OOS PASS
import numpy as np
import pandas as pd
...
def evaluate(bars, params):
    ...
    tk = float(params.get("tick", 0.005))
    sb = float(params.get("stop_buf_ticks", 5)) * tk
    bt = float(params.get("break_threshold_ticks", 2)) * tk
    mr = int(params.get("max_retest_bars", 48))
